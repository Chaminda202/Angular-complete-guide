...
export class AuthService {
  ...
  signUpT(request: SignUpRequest): Observable<SignUpResponse> {
    /*
    ...
    */
    ...
    ).pipe(catchError(erroRespo => {
      let errorMsg = 'Unknown error occured';
      if (erroRespo. error || erroRespo. error.error) {
        switch (erroRespo.error.error.message) {
          case 'EMAIL_EXISTS':
            errorMsg = 'The email address is already in use by another account';
            break;
          default:
            errorMsg = 'Error in firebase';
            break;
        }
      }
  ...
  loginT(request: SignUpRequest): Observable<SignUpResponse> {
    return this.http.post<SignUpResponse>(
